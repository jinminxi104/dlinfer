...
def AscendCudaGraphMixin_fill_buffers_cudagraph(
    ...
    **kwargs,
    ...
    # fill buffer
    input_buffers['input_ids'].fill_(0)
    ...
    #input_buffers["q_seqlens"].copy_(torch.arange(1, input_buffers["q_seqlens"].shape[0]+1))
    #input_buffers["q_seqlens"][num_tokens:] = num_tokens
    #print(input_buffers["q_seqlens"])

    input_buffers["kv_start_indices"].fill_(-1)
    input_buffers["kv_start_indices"][:batch_size] = kv_start_indices

    '''
    print(f"{batch_size}  {num_tokens}")
    print("pos")
    print(input_buffers["position_ids"])
    print("block")
    print(input_buffers["block_offsets"])
    print(input_buffers["block_offsets"].shape)
    print("kv start")
    print(input_buffers["kv_start_indices"])
    '''
...
class AscendGraphRunner(GraphRunner):
    ...
    def __call__(self, **kwargs):
        ...
        if not enable_graph:
            #print(f"eaggggggggggggggger", flush=True)
        ...
        if graph_key not in self._runner_map:
            #print(f"capturingggggggggg", flush=True)
            logger.error(f"graph key {graph_key} start")
            ...
            logger.error(f"graph key {graph_key} end")
            return None
        else:
            runner = self._runner_map[graph_key]
            output = runner.forward(**kwargs)
            torch.npu.synchronize()
        #print(f"replayyyyyyyyyyyyyyyy", flush=True)
        #output = runner.forward(**kwargs)
        return output
...
def clear_graph_params():
    ...
    try:
        ...
        global _graph_capture_sizes
        _graph_params = GraphParams(
            {size: [] for size in _graph_capture_sizes},
            {size: None for size in _graph_capture_sizes},
            {size: [] for size in _graph_capture_sizes},
            {size: [] for size in _graph_capture_sizes},
        )
